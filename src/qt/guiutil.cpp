...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BCC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no BeaconCoin: URI
...
{
    // Convert BeaconCoin:// to BeaconCoin:
    //
    //    Cannot handle this later, because BeaconCoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open BeaconCoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BeaconCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BeaconCoin.lnk
...
{
    return GetAutostartDir() / "BeaconCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a BeaconCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BeaconCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BeaconCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add BeaconCoin app to startup item list
