...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"bccaddress\"\n"
            "\nReturn information about the given bcc address.\n"
            "\nArguments:\n"
            "1. \"bccaddress\"     (string, required) The bcc address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"bccaddress\", (string) The bcc address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BCC address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are bcc addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) bcc address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"bccaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"bccaddress\"  (string, required) The bcc address to use for the signature.\n"
