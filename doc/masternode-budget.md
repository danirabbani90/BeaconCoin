...
BeaconCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<BeaconCoin_address\> \<monthly_payment_BeaconCoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ BCC, _12_ times over the course of a year totaling _24000_ BCC.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<BeaconCoin_address\> \<monthly_payment_BeaconCoin\> \<collateral_hash\>
...
After you get enough votes, execute `mnbudget projection` to see if you made it into the budget. If you the budget was finalized at this moment which proposals would be in it. Note: Proposals must be active at least 1 day on the network and receive 10% of the masternode network in yes votes in order to qualify (E.g. if there is 250000 masternodes, you will need 250 yes votes.)
...
When block `1000000` is reached you'll receive a payment for `1200` BCC.
 ...
 * vote-many          - Vote on a BeaconCoin initiative
 * vote-alias         - Vote on a BeaconCoin initiative
 * vote               - Vote on a BeaconCoin initiative/budget
